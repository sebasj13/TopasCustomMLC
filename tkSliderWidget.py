# -*- coding: utf-8 -*-
"""
Created on Wed Nov 24 09:23:36 2021

@author: Sebastian Schäfer | Modified from tkSliderWidget.py by MenxLi
...
class Slider(Frame):
    LINE_COLOR = "#476b6b"
    LINE_WIDTH = 3
    BAR_COLOR_INNER = "#9e210e"
    BAR_COLOR_OUTTER = "#9e210e"
    BAR_RADIUS = 10
    ...
    def getValues(self):
        values = [round(bar["Value"],1) for bar in self.bars]
        return values
    ...
    def __addBar(self, pos):
        ...
        id_outer = self.canv.create_rectangle(x+2,y+R,x-2,y-R, width = 5, fill = Slider.BAR_COLOR_OUTTER, outline = "")#create_oval(x-R,y-R,x+R,y+R, fill = Slider.BAR_COLOR_OUTTER, width = 2, outline = "")
        #id_inner = self.canv.create_rectangle(x,y-R,x,y+R, width = 2, outline = "")#create_oval(x-r,y-r,x+r,y+r, fill = Slider.BAR_COLOR_INNER, outline = "")x+2,y+R,x-2,y-R,
        if self.show_value:
            y_value = y#-Slider.BAR_RADIUS-8
            value = pos*(self.max_val - self.min_val)+self.min_val
            cur_values = self.getValues()
            
            if math.isclose(value,cur_values[0],rel_tol=0.5) == True:
                x_value = 30
            else:
                x_value = 720

            id_value = self.canv.create_text(x_value,y_value, text = format(value, Slider.DIGIT_PRECISION))
            id_box = self.canv.create_rectangle(x_value+13, y_value-6,x_value-13,y_value+6, fill = "white", )
            self.canv.tag_lower(id_box, id_value)
    ...
    def __moveBar(self, idx, pos):
        ...
        self.bars[idx]["Value"] = round(pos*(self.max_val - self.min_val)+self.min_val,1)
